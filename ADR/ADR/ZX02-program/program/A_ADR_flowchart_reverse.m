...
for cut_v =[1.5, 2]
    ...
    for method_num = 1:4
        ...
        for i = 1:T    
            tempdata = data{i}; 
            sub_cp = tempdata.c_p;
            sym1 = tempdata.index{1};%asia
            sym2 = tempdata.index{2};%us
            sym3 = tempdata.index{3};%比率    
            ...
            x3.data = 1./x3.data;
            if eq(method_num,1)
                [tref,yc,y,recorder,info] =ADR_method_reverse.sig1mod1(x2,x1,x3,sub_fee2,sub_fee1,W0,cut_v,sub_cp);    
            elseif eq(method_num,2)
                [tref,yc,y,recorder,info] =ADR_method_reverse.sig1mod2(x2,x1,x3,sub_fee2,sub_fee1,W0,cut_v,sub_cp);    
            elseif eq(method_num,3)
                [tref,yc,y,recorder,info] =ADR_method_reverse.sig2mod1(x2,x1,x3,sub_fee2,sub_fee1,W0,cut_v,sub_cp);   
            else
                [tref,yc,y,recorder,info] =ADR_method_reverse.sig2mod2(x2,x1,x3,sub_fee2,sub_fee1,W0,cut_v,sub_cp);
            end
...
%         rec = cat(1,rec,[temp2,temp1]);
    ...
    key_str = sprintf('reverse-支线任务2-ADR%s',datestr(now,'yyyymmdd'));
