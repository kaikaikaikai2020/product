...
[index_info,~,ib] = intersect(data00.index,index_info);
index_ticker=index_ticker(ib);
...
for index_id = 1:T
    ...
    stocks= cellfun(@(x) x(1:end-1),stocks,'UniformOutput',false);
    
    data0=data00.(index_sel);
    
    stks = get_pool_data_update(stocks,'2005-01-01','japan_TMP');
    stocks=stks.stocks;
    %load tempstks.mat
    tday = stks.tday;
    cl=stks.cl;
    op=stks.op;
    tref = stks.tref;
    tref = cellfun(@(x) x(1:10),tref,'UniformOutput',false);
    if size(tref,2)>size(tref,1)
        tref = tref';
    end
    
    op_adj=nan(length(data0.tref),size(cl,2));
    [stocks,ia,ib] = intersect(stocks,cellfun(@(x) x(3:end),data0.stocks,'UniformOutput',false),'stable');
