...
为了适应补充数据
%}
par_info = {"2330","TSM";"9984","SFTBY";'INFO','INFY'};
forex_pool = {'USDTWD','USDJPY','USDINR'};
r = [5,0.5,1];
data = load('data20201018.mat');
data = data.sub_data;
...
for isel = 1:size(par_info,1)
    ...
    ind = strcmp(data.stocks,ticker1);
    x1=[data.tref,num2cell([data.PX_OPEN(:,ind),data.PX_LAST(:,ind)])];
