...
%所有结果整合
%综合程序
%a38_kf_综合的pnl的图画两张-一个是我们现在有的，一个画一下最近三个月的。
addpath(genpath(fullfile(pwd,'jplv7')))
%updateA38data()
data0 = load('bloombergdata');

for remain_num = [inf,21*3]
    data = data0.data;
    if isinf(remain_num)        
        fn_str1 = '全部历史数据';
    else
        f_n = fieldnames(data0.data);
        for i = 1:length(f_n)
            temp = data.(f_n{i});
            temp = temp(end-remain_num:end,:);
            data.(f_n{i}) = temp;
        end
        fn_str1 = sprintf('近%d天数据',remain_num);
    end
    ...
    writetable(X,sprintf('A1_A38_KF_综合_%s_%s.csv',fn_str1,datestr(now,'yymmdd')))
end
