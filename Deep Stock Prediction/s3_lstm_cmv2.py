...
def generate_data(data, window=50):
    ...
    y_data = np.zeros(shape=(data_size - window, 2))
    for i in range(data_size - window):
        x_data[i] = data[i:i + window]
        y_data[i, 0] = data[i + window]
        y_data[i, 1] = data[i + window - 1]
    return x_data.reshape(-1, window, 1), y_data


def custom_loss(y_true, y_pred):
    y_true, y_pre = tf.reshape(y_true[:, 0], (-1, 1)), tf.reshape(y_true[:, 1], (-1, 1))
    print(y_pre.shape, y_pred.shape)
    bool_ind = (y_pred - y_pre) * (y_true - y_pre) < 0
    loss = tf.reduce_mean(tf.square(y_true[bool_ind] - y_pred[bool_ind]), axis=-1)
    return loss
...
for k, df in data_dict.items():
    ...
    model.compile(optimizer=keras.optimizers.Adam(lr=5 * 1e-3), loss=custom_loss)
    print('done')
    ...
    test_df.to_csv('./pred_data/lstmcm_%s.csv' % k)
    # plot
    test_df['mid'].plot()
    test_df['pred'].plot()
    plt.show()
