...
#HA配对交易
#tF '1.4.0'
#csi300 vs csi500 /csi300 vs sse50/csi500 vs sse50（指数和期货）
#select count(*) from yq_MktMFutdGet where contractObject="IH" and mainCon=1 limit 1;
...
from yq_toolsS45 import engine,engine_db_polygon
...
#导入数据
def import_I_F(ticker):
    if ticker in ['000300','000905','000016']:
        sql_str = """select tradeDate as `date`,closeIndex as close from yq_index where 
        symbol="%s" order by tradeDate"""
        eg = engine
    elif ticker in ['IC','IH','IF']:
        sql_str = """select tradeDate as `date`,closePrice as close from yq_MktMFutdGet where 
        contractObject="%s" and mainCon=1 order by tradeDate"""
        eg = engine
    elif ticker in ['SPY','QQQ']:
        sql_str = 'select tradeDate as `date`,closePrice as close from usastock_day where ticker = "%s" order by tradeDate';
        eg = engine_db_polygon
    x = pd.read_sql(sql_str % ticker,eg)
    if len(x)>0:
...
def do_cal(inputdata):
    ...
    if os.path.exists(com_fn2) or os.path.exists(com_fn3):
        return
    try:
        cal_HA_data(fn1,fn2)
    except:
...
if __name__ == '__main__':
    ...
    p1=['000300','000300','000905','IF','IF','IC','000300','000905','000016','SPY']
    p2=['000905','000016','000016','IC','IH','IH','IF','IC','IH','QQQ']
    
    fn_a = zip(p1,p2)
    pool = multiprocessing.Pool(min(num_core,len(p1))) 
