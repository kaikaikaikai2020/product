...
# %%
thread = 1/2
cgan_ensamble_model = joblib.load('./cgan_ensemble_model.pkl')
base_model = gsearch.best_estimator_
pred_em = np.array([cgan_model.best_estimator_.predict(x_test) for cgan_model in cgan_ensamble_model]).T
pred_y = (pred_em.sum(axis = 1) > (pred_em.shape[1] * thread )).astype(int)
# %%
plt.figure(figsize=(16, 8))
plt.plot((y_test.ravel() + 1).cumprod(), label='index')
plt.plot((pred_y * y_test.ravel() + 1).cumprod(), label='cgan')
