...
for ticker in tqdm(stock_list):
    ...
    data_pd = bbg.bdib(ticker, fld_list, prev, today, eventType='TRADE', interval = 60)
    ...
    if(len(temp_high)>1):
        ...
        result = result.append({'Bloomberg Code': ticker,'symbol': '>', 'level': temp_high[-2]}, ignore_index=True)
    
    current = data_pd['close'].iloc[-1]*low_gap
    minm = argrelextrema(data_pd['low'].values, np.less)  # (array([2, 5, 7]),)
    temp_low = data_pd['low'].values[maxm]
    ...
    if(len(temp_low)>1):
        ...
        result_low = result_low.append({'Bloomberg Code': ticker,'symbol': '<', 'level': temp_low[-2]}, ignore_index=True)    
