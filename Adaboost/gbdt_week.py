...
# %%
data_list = os.listdir('./week_data')
data_list = [i for i in data_list if 'csv' in i]
data_list.sort()


def get_train_test_path(begin_mon='2010-01', window=12, base_dir='./week_data'):
...
def read_fac_data(begin_mon='2010-01', window=12, base_dir='./week_data'):
...
def filter_by_adb(train_data, fac_cols, y_col):
    ...
    model = adaboost_weak_cls(n_models=50, Q=5)
...
(pd.Series(rets).fillna(0) + 1 - 0.005).cumprod().plot()
plt.show()
