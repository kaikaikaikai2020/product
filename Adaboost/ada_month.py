...
def get_train_test_path(begin_mon='2010-01', window=12, base_dir='./month_data'):
    ...
    # train_ind = data_list[beg_ind: beg_ind + window + 1] # 此处加一，表示将测试集也放置于模型中训练。
...
def processing_data(df, is_train=True):
    ...
    if is_train:
        ...
        return df_x[chs_up | chs_down], pd.DataFrame(df_y)
    else:
        df_x = df_x.apply(lambda x: processing_data_order(x))
...
for date in tqdm(data_list[:-window]):
    ...
    model = adaboost_weak_cls(n_models=50, Q=3)
    train_pred = model.fit(x_train.values, y_train.values)
    test_pred = model.predict(x_test)
    train_ic = spearmanr(train_pred.ravel(), y_train.values.ravel())[0]
    test_ic = spearmanr(test_pred.ravel(), df_ret['real_return'].apply(lambda x: 1 if x > 0 else -1).values)[0]
