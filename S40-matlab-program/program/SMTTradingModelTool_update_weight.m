...
% 交易模拟--选取相关性前M个匹配
function [tradeYield,result,tradeDetail,yearDetail,h,assure_ratio] =SMTTradingModelTool_update_weight(hsClose,hsOpen,hsDate,ifClose,ifOpen,ifDate,P)
    %% 参数设置
    feeOpen=P.feeOpen;
    feeClose=P.feeClose;
    ...
    elseif stopMethod==3
        ...
        tradeYield=tradeSimulate3(fractalPostion,outTradeClose,outTradeDate,feeOpen,feeClose,dayMin,outTradeOpen,assure_ratio);
    end
...
weight_use = tradeYield(:,end);
%只多仓 修正
if eq(trade_mode,1)
    Yield=tradeYield(:,2);
    Yield(tradeYield(:,3)<0)=0;
    noYield=Yield;
    noYield(tradeYield(:,3)>0)=tradeYield(tradeYield(:,3)>0,2)+(feeOpen+feeClose)*weight_use(tradeYield(:,3)>0);
else
    Yield=tradeYield(:,2);
    noYield=Yield+(feeOpen+feeClose)*weight_use;
end
...
subplot(2,1,2)
dateV=datevec(tradeYield(:,1));
index=[1;find(diff(dateV(:,1)));length(tradeYield(:,1))];
yearS=dateV(index(2:end));
yearRate=(cumYield(index(2:end),1)+1)./(cumYield(index(1:end-1),1)+1)-1;
redRate=yearRate;
redRate(redRate<=0)=0;
blueRate=yearRate;
...
function tradeYield=tradeSimulate3(fractalPostion,outTradeClose,outTradeDate,feeOpen,feeClose,dayMin,outTradeOpen,assure_ratio)
%% 函数说明
% 根据给出的仓位对股指进行交易
...
for i=ind:tradeNum
    ...
    if isempty(index)==0
        if mod(fractalPostion(index(1),6),dayMin)==0
            tradeYield(i,1)=(fractalPostion(index(1),2)*(outTradeClose(fractalPostion(index(1),6))/outTradeClose(fractalPostion(index(1),3))-1)-feeOpen-feeClose)*assure_ratio(index(1));
            tradeYield(i,2)=fractalPostion(index(1),2);
            tradeYield(i,3)=outTradeClose(fractalPostion(index(1),3));
            tradeYield(i,4)=outTradeClose(fractalPostion(index(1),6));
            tradeYield(i,5)=fractalPostion(index(1),3);
            tradeYield(i,6)=fractalPostion(index(1),6);
            tradeYield(i,7)=assure_ratio(index(1));
        else
            ...
            if -openRate>fractalPostion(index(1),4)
                tradeYield(i,1)=(openRate-feeOpen-feeClose)*assure_ratio(index(1));
                tradeYield(i,2)=fractalPostion(index(1),2);
                tradeYield(i,3)=outTradeClose(fractalPostion(index(1),3));
                ...
                tradeYield(i,1)=(-fractalPostion(index(1),4)-feeOpen-feeClose)*assure_ratio(index(1));
                tradeYield(i,2)=fractalPostion(index(1),2);
                tradeYield(i,3)=outTradeClose(fractalPostion(index(1),3));
                tradeYield(i,4)=tradeYield(i,3)*(1-tradeYield(i,2)*fractalPostion(index(1),4));
                tradeYield(i,5)=fractalPostion(index(1),3);
