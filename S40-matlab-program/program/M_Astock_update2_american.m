...
%sql_str = 'select * from index_comp_price.american_djia where Ticker = 'C' order by tradingdate'
for i_sym =1:T_symbols
    ...
    %try 
        ...
        P.tradeMin=210;%使用早盘120分钟K线数据进行分形匹配
        P.dayMin=390;%每个交易日共240根1分钟K线
        P.M=20;%找M个最为相似的交易日
        ...
        sql_str = ['select year(tradingdate),month(tradingdate),day(tradingdate),',...
            ...
            'where Ticker = ''%s'' order by tradingdate '];
