...
% 交易模拟--选取相关性前M个匹配
function [tradeYield,result,tradeDetail,yearDetail,h] =SMTTradingModelTool_update2(hsClose,hsOpen,hsDate,ifClose,ifOpen,ifDate,P)
    %% 参数设置
    feeOpen=P.feeOpen;
    feeClose=P.feeClose;
    ...
    if eq(trade_mode,1)
        ...
        tradeYield(:,2) = Yield;
        tradeYield(tradeYield(:,3)<0,3)=0;
    end
    % 结果输出
    [result,h]=outResultCaculate(tradeYield,feeOpen,feeClose);
...
for i=1:length(outDay)
    ...
    for ind_i=1:M
        ...
        corrRecord{i,1}(ind_i,5) = matchClose(allDay(j+1)-1,1)/matchClose(allDay(j)+tradeMin-1,1)-1;
    end
end
end
...
for i=1:length(outDay)-1
    ...
    chg_avg_long = mean(corrRecord{i,1}(longIndex,5));
    chg_avg_all = mean(corrRecord{i,1}(:,5));
    %增加了一个判断
    if length(longIndex)/lenCorr>muchPara
        % 历史匹配分形--做多
        fractalPostion(ind_j,1)=floor(outMatchDate(outDay(i)));%交易日期
        if chg_avg_long>cut_return
...
function [result1,h]=outResultCaculate(tradeYield,feeOpen,feeClose)
%% 函数说明
% 输出当前交易结果及统计数据
%%
