%不并行，直接使用in的方法
function x = get_pool_data_csi_adj2(stocks,t0,pool)
    if nargin <3
        pool = 'us';
    end
    if nargin <2
    ...
    elseif strcmp(pool,'csi')
        ...
        sql_str = ['select tradeDate,openPrice as op,highPrice as hi,lowPrice as lo,',...
        'closePrice as cl from data_pro.main_indexfactor_S42 where index_id="',pool,'" and tradeDate>="%s" order by tradeDate'];
    end
