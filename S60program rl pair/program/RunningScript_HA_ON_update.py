...
def cal_HA_data(fn1,fn2):
    ...
    #
    sql_hk = 'select tradeDate,closePrice from mkthkequdgets54 where ticker="%s" and tradeDate>="2010-01-01" order by tradeDate'
    sql_A = 'select tradeDate,closePrice from yq_dayprice where symbol="%s" order by tradeDate'
    x = pd.read_sql(sql_hk % fn1,engine)
    x.tradeDate = x.tradeDate.astype(str)
    x.columns=['date','close']
    y = pd.read_sql(sql_A % fn2,engine)
    y.tradeDate = y.tradeDate.astype(str)
    y.columns=['date','close']
    #对齐
...
if __name__ == '__main__':
    ...
    x.f1 = x.f1.apply(lambda x:'%0.5d' % (int(x)))
    x.f2 = x.f2.apply(lambda x:'%0.6d' % (int(x)))
