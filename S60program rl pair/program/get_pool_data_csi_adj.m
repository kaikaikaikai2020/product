%不并行，直接使用in的方法
function x = get_pool_data_csi_adj(stocks,t0,pool)
    if nargin <3
        pool = 'us';
    end
    if nargin <2
    ...
    if strcmpi(pool,'us')
        ...
        'closePrice as cl from yuqerdata.MktEqudAdjAfGetF1S53 where isOpen=1 and ticker in (%s) and tradeDate>="%s" order by tradeDate'];
    end
