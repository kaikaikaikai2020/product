...
%}
classdef bac_result_S76_update<handle
    methods
    ...
    methods(Static)
        function update_signal()
            sprintf('开始执行42升级信号程序')
            dos('python bac_toolS42.py')
        end
        function [H,Y,info] = bac_re_other()
            key_str = sprintf('S42 双底策略指数成分股组合%s',datestr(now,'yyyymmdd'));
            %forex-day,as51,topix
            index_pool = {'hsce','US','HK','forex_day','as51','topix','twse','hk_ggt'};
            %index_pool = {'HK'};
